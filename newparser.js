...
// utility to create a uint24 from 3 bytes
const parseUint24 = (data) => (
  (data[0] << 16) |
  (((1 << 16) - 1) & (data[1] << 8)) |
  ((1 << 8) - 1) &
  data[2]
...
// stupid, almost completely minimal unsigned integer 3-byte array
class UInt24Array {
  static from (a) {
    const out = new Uint32Array(a.length / 3)
    let ai = 0
    for (let i = 0; i < a.length; a += 3) {
      out[ai] = parseUint24(a.slice(i, i + 3))
      ai++
    }
    return out
  }
...
class Mindwave {
  ...
  static parse (buffer) {
    ...
    while (msg.length) {
      ...
      switch (msg[0]) {
        ...
        case HEART_RATE:
          out.heart_rate = getScaledValue(msg[1], 0, 255)
          msg = msg.slice(1)
          break
  ...
  async connectBtWeb () {
    if (typeof navigator === 'undefined' || typeof navigator.bluetooth === 'undefined') {
      throw new Error('Your browser does not support bluetooth.')
    }
    // TODO
  }
  ...
  // MAC = native bluetooth
  // COM or /dev/X is native serial path
  // no path triggers browser
  connect (path) {
    if (!path) {
    ...
    if (path.includes('/dev') || path.includes('COM')) {
